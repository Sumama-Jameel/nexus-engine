...
func TestCheckLock_LockFileHeldByAnotherProcess(t *testing.T) {
	...
	// Clean up
	_ = os.Remove(lockPath)          //nolint:errcheck // test cleanup
	_ = os.Remove("/var/lib/pacman") //nolint:errcheck // test cleanup
}
...
func TestCheckLock_PermissionDeniedOnLockFile(t *testing.T) {
	...
	_ = f.Close() //nolint:errcheck // test setup

	// Remove all permissions
	_ = os.Chmod(lockPath, 0000) //nolint:errcheck // test setup

	// checkLock should still return true (permission denied → continue → no lock detected)
	result := checkLock("arch")

	// Restore permissions for cleanup
	_ = os.Chmod(lockPath, 0644)     //nolint:errcheck // test cleanup
	_ = os.Remove(lockPath)          //nolint:errcheck // test cleanup
	_ = os.Remove("/var/lib/pacman") //nolint:errcheck // test cleanup
...
func TestCheckLockFiles_LockHeldByAnotherProcess(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		_ = syscall.Flock(int(f.Fd()), syscall.LOCK_EX) //nolint:errcheck // test setup
		close(lockAcquired)
		time.Sleep(300 * time.Millisecond)
		_ = syscall.Flock(int(f.Fd()), syscall.LOCK_UN) //nolint:errcheck // test cleanup
...
func TestCheckLockFiles_PermissionDenied(t *testing.T) {
	...
	_ = os.Chmod(lockPath, 0000)                    //nolint:errcheck // test setup
	defer func() { _ = os.Chmod(lockPath, 0644) }() //nolint:errcheck // test cleanup
...
func TestCheckLockFiles_MultipleFilesOneHeld(t *testing.T) {
	...
	f1.Close() //nolint:errcheck // test setup
	...
	go func() {
		defer wg.Done()
		_ = syscall.Flock(int(f2.Fd()), syscall.LOCK_EX) //nolint:errcheck // test setup
		close(lockAcquired)
		time.Sleep(300 * time.Millisecond)
		_ = syscall.Flock(int(f2.Fd()), syscall.LOCK_UN) //nolint:errcheck // test cleanup
		_ = f2.Close()                                   //nolint:errcheck // test cleanup
	}()
