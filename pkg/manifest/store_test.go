...
func TestProfileStore_Initialize_ExistingProfileReadFails(t *testing.T) {
	...
	defer func() { _ = os.Chmod(path, 0644) }() //nolint:gosec // restore for cleanup
...
func TestProfileStore_Initialize_WriteFails(t *testing.T) {
	...
	if chmodErr := os.Chmod(store.dir, 0555); chmodErr != nil { //nolint:gosec
		t.Fatalf("failed to chmod profiles dir: %v", chmodErr)
	}
	defer func() { _ = os.Chmod(store.dir, 0755) }() //nolint:gosec // restore for cleanup
...
func TestNewProfileStore_MkdirAllFails(t *testing.T) {
	...
	if err := os.MkdirAll(readOnlyDir, 0555); err != nil { //nolint:gosec // intentional restrictive perms for test
...
func TestProfileStore_SaveRegistry_RenameFails(t *testing.T) {
	...
	if mkdirErr := os.MkdirAll(filepath.Join(store.dir, "registry.json"), 0755); mkdirErr != nil { //nolint:gosec // test directory
		t.Fatalf("failed to create blocking directory: %v", mkdirErr)
	}
...
func TestNewProfileStore_CorruptRegistry(t *testing.T) {
	...
	if err := os.MkdirAll(profilesDir, 0755); err != nil { //nolint:gosec // test directory
