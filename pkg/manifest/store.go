...
// defaultHTTPClient is the HTTP client used by FetchProfile. It uses
// an SSRF-safe transport that rejects connections to private/reserved
// IP ranges, preventing DNS rebinding attacks. Tests can replace it
// with a custom client that trusts a test TLS server's certificate.
var defaultHTTPClient = &http.Client{
	Timeout:   30 * time.Second,
	Transport: netutil.NewSSRFSafeTransport(),
}
