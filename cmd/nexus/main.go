...
func runWSLImport(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		if outputJSON {
			_ = jsonOutput(map[string]interface{}{"error": err.Error(), "available_images": wsl.DefaultRootFSRegistry()})
		} else {
			fmt.Printf("\n  ⛔ %v\n\n", err)
		}
	...
	if err := wsl.ValidateInstallPath(installPath); err != nil {
		if outputJSON {
			_ = jsonOutput(map[string]interface{}{"error": fmt.Sprintf("invalid install path: %v", err)})
		} else {
