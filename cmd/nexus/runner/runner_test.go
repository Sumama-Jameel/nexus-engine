...
// createTestProfileStore creates a ProfileStore in a temp directory with
// a single test profile pre-saved. Used by ApplyProfile and
// ResolveProfilePackages tests that need a non-nil store with content.
func createTestProfileStore(t *testing.T, name, content string) (*manifest.ProfileStore, error) {
	t.Helper()
	store, err := manifest.NewProfileStore()
	if err != nil {
		return nil, err
	}
	if err := store.SaveProfile(name, []byte(content), manifest.SourceLocal); err != nil {
		return nil, err
	}
	return store, nil
}
...
`)
	if err != nil {
		t.Fatalf("createTestProfileStore: %v", err)
	}
	deps.ProfileStore = store

	profile, packages, err := deps.ResolveProfilePackages("test-profile")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if profile == nil {
...
func TestResolveProfilePackages_LoadError(t *testing.T) {
	deps := createTestDependencies(t)
	store, err := createTestProfileStore(t, "real-profile", `name: real-profile
version: "1.0.0"
targets:
  - family: debian
    packages: [git]
`)
	...
	// Try to load a non-existent profile — should fail
	_, _, err = deps.ResolveProfilePackages("nonexistent-profile")
	if err == nil {
...
// ---------------------------------------------------------------------------
// ApplyProfile — with a real profile store
// ---------------------------------------------------------------------------

func TestApplyProfile_Success(t *testing.T) {
	deps := createTestDependencies(t)
	store, err := createTestProfileStore(t, "test-profile", `name: test-profile
version: "1.0.0"
targets:
...
`)
	...
	result, profile, err := deps.ApplyProfile(context.Background(), "test-profile")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if profile == nil {
...
`)
	...
	_, _, err = deps.ApplyProfile(context.Background(), "nonexistent-profile")
	if err == nil {
